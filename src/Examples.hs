...
data CollatzTree_ a
  = OneBranch (Int, Int) a
  | TwoBranch (Int, Int) a a
    deriving (Functor)

type CollatzTree = Fix CollatzTree_

hyloCollatz :: Int -> (Int, Int) -> [[(Int,Int)]]
hyloCollatz depth = fmap (filter (not . (`elem` [0,1,2,4]) . snd)) . take depth . T.levels . hylo g f
  where f :: (Int,Int) -> CollatzTree_ (Int, Int)
        f (colt, n) | n `mod` 3 == 2 = TwoBranch (colt,n) (colt+1, (n - 1) `div` 3) (colt+1, 2 * n)
